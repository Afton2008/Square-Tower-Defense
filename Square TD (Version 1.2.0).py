...
def info_screen():
    ...
    # ======================
        ...
        v5_title = font_mid.render("Version 1.2.0 - 14 Februari 2026", True, (100, 200, 255))
        screen.blit(v5_title, (WIDTH//2 - v5_title.get_width()//2, y_offset))
        y_offset += 50

        v5_lines = [
            "Perbaikan Event Winter dan Chapter I",
            "Perbaikan Klik tombol + UI",
            "Penguncian Chapter",
            "+ Format angka",
            "Pembaruan Bar Boss HP",
            "Penambahan Spin dan Boost",
            "+5 Tower baru dan +2 Tower Eksklusif",
    ...
    # BUTTON
    # ======================
    back_btn = draw_button("Kembali", WIDTH//2 - 200, 680, 400, 80, mouse_pos)
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if back_btn.collidepoint(mx, my):
                game_state = "intro"
            elif prev_btn and prev_btn.collidepoint(mx, my):
                info_page -= 1
...
def home_screen():
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if btn_inventory.collidepoint(mx, my):
                game_state = "inventory"
            elif btn_story.collidepoint(mx, my):
                game_state = "menu"
            elif btn_event.collidepoint(mx, my):
                event_menu()
                return
...
def perform_spin():
    weighted_rarities = []
    for rarity, data in SPIN_REWARDS.items():
        weighted_rarities.extend([rarity] * data["weight"])

    selected_rarity = random.choice(weighted_rarities)

    items = SPIN_REWARDS[selected_rarity]["items"]
    selected_item = random.choice(items)
    
    return {
        "rarity": selected_rarity,
        "item": selected_item,
        "color": SPIN_REWARDS[selected_rarity]["color"]
    }

def apply_spin_reward(result):
    item = result["item"]
    
    if item["type"] == "shape":
        save["shape"] = save.get("shape", 0) + item["amount"]
        save_data(save)
    
    elif item["type"] == "money":
        temp_money_boosts.append({
            "amount": item["amount"],
            "used": False
        })
        
    elif item["type"] == "boost":
        expires = pygame.time.get_ticks() + item["duration"]
        active_boosts.append({
            "type": "small" if item["amount"] == 1.5 else "large",
        ...
        if tower_name in towers_data:
            towers_data[tower_name]["owned"] = True
            save_inventory()
            
    elif item["type"] == "nothing":
        pass

def apply_temp_money_boost(game_state_data):
    total_bonus = 0
    for boost in temp_money_boosts[:]:
        if not boost["used"]:
            total_bonus += boost["amount"]
            boost["used"] = True
    
    if total_bonus > 0:
        game_state_data["money"] += total_bonus
        temp_money_boosts[:] = [b for b in temp_money_boosts if not b["used"]]
        return total_bonus
    return 0

def get_active_boost():
    now = pygame.time.get_ticks()
    active_boosts[:] = [b for b in active_boosts if b["expires"] > now]
    
    if not active_boosts:
        return None

    boost = max(active_boosts, key=lambda b: b["multiplier"])
    time_left = (boost["expires"] - now) / 1000.0
...
def apply_boost_to_tower(tower, boost_info):
    if not hasattr(tower, '_original_damage'):
        tower._original_damage = tower.damage
    if not hasattr(tower, '_original_fire_rate'):
        tower._original_fire_rate = tower.fire_rate
    
    if not boost_info:
        tower.damage = tower._original_damage
        tower.fire_rate = tower._original_fire_rate
        tower._current_boost_type = None
        return

    current_boost = getattr(tower, '_current_boost_type', None)
    if current_boost == boost_info["type"]:
        return
    
    multiplier = boost_info["multiplier"]
    boost_type = boost_info["type"]

    tower.damage = tower._original_damage
    tower.fire_rate = tower._original_fire_rate
    
    # Small boost: Damage only
    if boost_type == "small":
        tower.damage = int(tower._original_damage * multiplier)
    
    # Big boost: Damage + Fire Rate
    elif boost_type == "large":
        tower.damage = int(tower._original_damage * multiplier)
        tower.fire_rate = tower._original_fire_rate / multiplier
    
    tower._current_boost_type = boost_type

def draw_boost_indicator(screen):
    boost = get_active_boost()
    if not boost:
        return
    ...
    glow_alpha = int(100 + 50 * abs(math.sin(pygame.time.get_ticks() / 300)))
    glow_surf = pygame.Surface((box_w + 10, box_h + 10), pygame.SRCALPHA)
    if boost["type"] == "small":
        glow_color = (50, 200, 50, glow_alpha)
        box_color = (30, 150, 30)
        boost_name = "SMALL BOOST"
    else:
        glow_color = (255, 165, 0, glow_alpha)
        box_color = (200, 100, 0)
        boost_name = "BIG BOOST"
    
    pygame.draw.rect(glow_surf, glow_color, (0, 0, box_w + 10, box_h + 10), border_radius=15)
    screen.blit(glow_surf, (box_x - 5, box_y - 5))
    
    pygame.draw.rect(screen, box_color, (box_x, box_y, box_w, box_h), border_radius=12)
    pygame.draw.rect(screen, (255, 255, 255), (box_x, box_y, box_w, box_h), 3, border_radius=12)
 
    icon_x = box_x + 15
    icon_y = box_y + box_h // 2 - 15
    pygame.draw.circle(screen, (255, 255, 100), (icon_x + 15, icon_y + 15), 18)
    ...
    boost_text = font_mid.render(f" {boost_name}", True, (255, 255, 255))
    screen.blit(boost_text, (icon_x + 40, box_y + 12))

    mult_text = font_small.render(f"{boost['multiplier']}x ", True, (255, 255, 100))
    screen.blit(mult_text, (icon_x + 40, box_y + 45))
    
    if boost["type"] == "small":
        effect_text = "Damage"
    else:
        effect_text = "Damage + Fire Rate"
    effect_surf = pygame.font.Font(None, 28).render(effect_text, True, (200, 200, 200))
    screen.blit(effect_surf, (icon_x + 100, box_y + 47))

    minutes = int(boost["time_left"] // 60)
    seconds = int(boost["time_left"] % 60)
    time_text = pygame.font.Font(None, 28).render(f"{minutes}:{seconds:02d}", True, (255, 255, 255))
    screen.blit(time_text, (box_x + box_w - time_text.get_width() - 15, box_y + box_h // 2 - time_text.get_height() // 2))

def draw_spin_animation(screen, progress):
    center_x = WIDTH // 2
    ...
    pygame.draw.rect(screen, (40, 40, 60), (container_x, container_y, container_w, container_h), border_radius=20)
    pygame.draw.rect(screen, (100, 100, 150), (container_x, container_y, container_w, container_h), 5, border_radius=20)

    title = font_big.render("SPINNING...", True, (255, 255, 100))
    screen.blit(title, (center_x - title.get_width()//2, container_y + 30))
    ...
    if progress < 0.7:
        scroll_speed = progress * 15
    else:
        scroll_speed = 10.5 - (progress - 0.7) * 30
    
    offset = (pygame.time.get_ticks() * scroll_speed / 100) % len(all_rewards)

    item_width = 140
    item_height = 100
    visible_items = 5
    start_x = center_x - (visible_items * item_width) // 2
    roulette_y = center_y - item_height // 2

    for i in range(visible_items + 2):
        idx = int((i + offset)) % len(all_rewards)
        reward = all_rewards[idx]
        
        item_x = start_x + i * item_width - (offset % 1) * item_width

        alpha = 255
        if i == 0 or i == visible_items + 1:
            alpha = 100
        
        item_surface = pygame.Surface((item_width - 10, item_height), pygame.SRCALPHA)
        item_surface.fill((*reward["color"], alpha))
        screen.blit(item_surface, (item_x, roulette_y))
        
        pygame.draw.rect(screen, WHITE, (item_x, roulette_y, item_width - 10, item_height), 2)
 
        name_font = pygame.font.Font(None, 24)
        name_lines = reward["name"].split()
        
        y_offset = roulette_y + 20
        for line in name_lines:
            name_surf = name_font.render(line, True, WHITE)
            screen.blit(name_surf, (item_x + (item_width - 10)//2 - name_surf.get_width()//2, y_offset))
            y_offset += 25

        rarity_surf = pygame.font.Font(None, 20).render(reward["rarity"], True, reward["color"])
        screen.blit(rarity_surf, (item_x + (item_width - 10)//2 - rarity_surf.get_width()//2, roulette_y + item_height - 25))

    indicator_x = center_x
    indicator_y = roulette_y - 30
    ...
    bar_y = container_y + container_h - 60
    
    pygame.draw.rect(screen, (60, 60, 80), (bar_x, bar_y, bar_width, bar_height), border_radius=10)
    pygame.draw.rect(screen, (100, 255, 100), (bar_x, bar_y, int(bar_width * progress), bar_height), border_radius=10)

def draw_spin_result(screen, result):
    center_x = WIDTH // 2
    center_y = HEIGHT // 2
    ...
    container_w, container_h = 600, 450
    container_x = center_x - container_w // 2
    container_y = center_y - container_h // 2
    
    pygame.draw.rect(screen, (40, 40, 60), (container_x, container_y, container_w, container_h), border_radius=20)
    pygame.draw.rect(screen, result["color"], (container_x, container_y, container_w, container_h), 8, border_radius=20)

    banner_h = 80
    pygame.draw.rect(screen, result["color"], (container_x, container_y, container_w, banner_h))
    rarity_text = font_big.render(result["rarity"].upper(), True, WHITE)
    screen.blit(rarity_text, (center_x - rarity_text.get_width()//2, container_y + 20))

    item = result["item"]

    icon_size = 120
    icon_x = center_x - icon_size // 2
    icon_y = container_y + 120
    pygame.draw.rect(screen, result["color"], (icon_x, icon_y, icon_size, icon_size), border_radius=15)
    pygame.draw.rect(screen, WHITE, (icon_x, icon_y, icon_size, icon_size), 4, border_radius=15)

    item_name = font_mid.render(item["name"], True, WHITE)
    screen.blit(item_name, (center_x - item_name.get_width()//2, icon_y + 140))

    desc_y = icon_y + 200
    if item["type"] == "shape":
        desc = f"You got {item['amount']} Shape!"
    elif item["type"] == "money":
        desc = f"+{item['amount']} Money for next game!"
        desc2 = f"(Used once, then resets)"
        desc2_surf = font_small.render(desc2, True, (180, 180, 180))
        screen.blit(desc2_surf, (center_x - desc2_surf.get_width()//2, desc_y + 35))
    elif item["type"] == "boost":
        boost_type = "Small" if item["amount"] == 1.5 else "Big"
        desc = f"{boost_type} Boost for {item['duration']//60000} minutes!"
    elif item["type"] == "tower":
        desc = f"New tower unlocked!"
    else:
        desc = "Better luck next time!"
    
    desc_surf = font_small.render(desc, True, (200, 200, 200))
    screen.blit(desc_surf, (center_x - desc_surf.get_width()//2, desc_y))

    continue_text = font_small.render("Click anywhere to continue", True, (150, 150, 150))
    screen.blit(continue_text, (center_x - continue_text.get_width()//2, container_y + container_h - 40))
...
def inventory_screen():
    global game_state, inventory_page, spin_state, spin_animation_progress, spin_result, spin_animation_start_time

    mouse_pos = pygame.mouse.get_pos()
    screen.fill((30, 30, 50))

    if spin_state == "spinning":
        elapsed = pygame.time.get_ticks() - spin_animation_start_time
        spin_animation_progress = min(elapsed / SPIN_DURATION, 1.0)
        
        draw_spin_animation(screen, spin_animation_progress)
        
        if spin_animation_progress >= 1.0:
            spin_state = "result"
            apply_spin_reward(spin_result)
    ...
    elif spin_state == "result":
        draw_spin_result(screen, spin_result)
    ...
    shape_text = font_small.render(f"Shape: {format_number(save.get('shape', 0))}", True, WHITE)
    screen.blit(shape_text, (WIDTH - shape_text.get_width() - 40, 20))

    unused_boosts = [b for b in temp_money_boosts if not b["used"]]
    if unused_boosts:
        total_bonus = sum(b["amount"] for b in unused_boosts)
        boost_count = len(unused_boosts)
        boost_text = font_small.render(f"Money Boost: +{format_number(total_bonus)} ({boost_count}x)", True, (255, 215, 0))
        screen.blit(boost_text, (WIDTH - boost_text.get_width() - 40, 55))
        
        info_text = pygame.font.Font(None, 24).render("(Applied to next game)", True, (200, 200, 200))
        screen.blit(info_text, (WIDTH - info_text.get_width() - 40, 85))

    current_boost = get_active_boost()
    if current_boost:
        minutes = int(current_boost["time_left"] // 60)
        seconds = int(current_boost["time_left"] % 60)
        
        if current_boost["type"] == "small":
            boost_name = "Small Boost Active"
            boost_color = (50, 200, 50)
            boost_effect = f"{current_boost['multiplier']}x Damage"
        else:
            boost_name = "Big Boost Active"
            boost_color = (255, 165, 0)
            boost_effect = f"{current_boost['multiplier']}x Damage + Fire Rate"
        
        y_offset = 115 if unused_boosts else 55
        
        boost_active_text = font_small.render(f"{boost_name}", True, boost_color)
        screen.blit(boost_active_text, (WIDTH - boost_active_text.get_width() - 40, y_offset))
        
        effect_text = pygame.font.Font(None, 24).render(boost_effect, True, (200, 200, 200))
        screen.blit(effect_text, (WIDTH - effect_text.get_width() - 40, y_offset + 30))
        
        time_text = pygame.font.Font(None, 24).render(f"Time: {minutes}:{seconds:02d}", True, (255, 255, 100))
        screen.blit(time_text, (WIDTH - time_text.get_width() - 40, y_offset + 55))

    spin_btn_w, spin_btn_h = 250, 90
    spin_btn_x = 40
    spin_btn_y = 20

    glow_pulse = abs(math.sin(pygame.time.get_ticks() / 500)) * 0.5 + 0.5
    glow_color = tuple(int(c * glow_pulse + 100 * (1 - glow_pulse)) for c in RARITY_MYTHIC)

    for i in range(3):
        alpha = int(50 * (1 - i/3))
        glow_surf = pygame.Surface((spin_btn_w + i*10, spin_btn_h + i*10), pygame.SRCALPHA)
        pygame.draw.rect(glow_surf, (*glow_color, alpha), (0, 0, spin_btn_w + i*10, spin_btn_h + i*10), border_radius=15)
        screen.blit(glow_surf, (spin_btn_x - i*5, spin_btn_y - i*5))

    spin_btn_rect = pygame.Rect(spin_btn_x, spin_btn_y, spin_btn_w, spin_btn_h)
    
    if spin_btn_rect.collidepoint(mouse_pos):
        btn_color = tuple(min(c + 30, 255) for c in RARITY_MYTHIC)
    else:
        btn_color = RARITY_MYTHIC
    
    pygame.draw.rect(screen, btn_color, spin_btn_rect, border_radius=15)
    pygame.draw.rect(screen, (255, 255, 255), spin_btn_rect, 4, border_radius=15)

    spin_text = font_mid.render("SPIN", True, WHITE)
    screen.blit(spin_text, (spin_btn_x + spin_btn_w//2 - spin_text.get_width()//2, spin_btn_y + 15))
    
    cost_text = font_small.render(f"Cost: {SPIN_COST} Shape", True, (255, 255, 100))
    screen.blit(cost_text, (spin_btn_x + spin_btn_w//2 - cost_text.get_width()//2, spin_btn_y + 55))
    ...
    for i, (name, data) in enumerate(page_items):
        ...
        if name in ["Void Pulse (3K)", "Slingdrone (420)"]:
            if name == "Void Pulse (3K)":
                highlight_color = RARITY_MYTHIC
            else:
                highlight_color = RARITY_RARE
            pygame.draw.rect(screen, (*highlight_color, 50), (x-10, y-10, 400, 140), border_radius=10)
        
        txt = font_mid.render(name, True, WHITE)
        screen.blit(txt, (x, y))

        is_frost_blast = (name == "Frost Blast (1,8K)")
        is_spin_only = (name in ["Void Pulse (3K)", "Slingdrone (420)"])
        
        if owned:
            buy_btn = draw_button("Owned", x, y + 50, 250, 80, mouse_pos)
            buttons.append((name, buy_btn, owned, price, is_frost_blast, is_spin_only))
        elif is_frost_blast:
            buy_btn = draw_button("Event Reward", x, y + 50, 250, 80, mouse_pos)
            buttons.append((name, buy_btn, owned, price, is_frost_blast, is_spin_only))
        elif is_spin_only:
            buy_btn = draw_button("Spin Only", x, y + 50, 250, 80, mouse_pos)
            buttons.append((name, buy_btn, owned, price, is_frost_blast, is_spin_only))
        else:
            buy_btn = draw_button(f"Buy ({format_number(price)})", x, y + 50, 250, 80, mouse_pos)
            buttons.append((name, buy_btn, owned, price, is_frost_blast, is_spin_only))
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if spin_btn_rect.collidepoint(mx, my):
                if save.get("shape", 0) >= SPIN_COST:
                    save["shape"] -= SPIN_COST
                    save_data(save)
                    
                    spin_result = perform_spin()
                    spin_state = "spinning"
                    spin_animation_start_time = pygame.time.get_ticks()
                    spin_animation_progress = 0
                else:
                    print("Shape tidak cukup untuk spin!")
                continue

            for name, rect, owned, price, is_frost_blast, is_spin_only in buttons:
                if rect.collidepoint(mx, my):
                    if is_frost_blast and not owned: 
                        show_frost_blast_info_popup()
                    elif is_spin_only and not owned:
                        show_spin_only_info_popup(name)
                    elif not owned and not is_frost_blast and not is_spin_only:
                        if save["shape"] >= price:
                            save["shape"] -= price
            ...
            if btn_next and btn_next.collidepoint(mx, my):
                inventory_page += 1
            elif btn_prev and btn_prev.collidepoint(mx, my):
                inventory_page -= 1
            elif btn_back.collidepoint(mx, my):
                inventory_page = 0
                game_state = "home"
...
def show_spin_only_info_popup(tower_name):
    popup_running = True
    
    if tower_name == "Void Pulse (3K)":
        rarity = "MYTHIC"
        rarity_color = RARITY_MYTHIC
        chance = "1%"
        description = [
            "The ultimate tower!",
            "Devastating area damage",
            "Slowdown effect on enemies",
            "",
            f"Only obtainable from SPIN",
            f"Chance: {chance}"
        ]
    else:
        rarity = "RARE"
        rarity_color = RARITY_RARE
        chance = "15%"
        description = [
            "Catapult tower with splash damage",
    ...
    while popup_running:
        ...
        pygame.draw.rect(screen, rarity_color, (box_x, box_y, box_w, box_h), 10)
        ...
        title = font_big.render(tower_name.split("(")[0].strip(), True, rarity_color)
        screen.blit(title, (WIDTH//2 - title.get_width()//2, icon_y + 100))

        lock_text = font_mid.render(f"{rarity} - LOCKED", True, (255, 200, 100))
        screen.blit(lock_text, (WIDTH//2 - lock_text.get_width()//2, icon_y + 160))

        info_y = icon_y + 220
        
        for i, text in enumerate(description):
            if "Only obtainable" in text or "Chance:" in text:
                color = (255, 255, 100)
            else:
                color = (200, 220, 255)
...
def show_frost_blast_info_popup():
    popup_running = True
    ...
    while popup_running:
        ...
        title = font_big.render("Frost Blast", True, (160, 220, 255))
        screen.blit(title, (WIDTH//2 - title.get_width()//2, icon_y + 100))
...
class Tower:
    ...
    def get_furthest_enemy(self, enemies):
        """Get enemy that is furthest along the path within range"""
        target = None
        best_progress = -999999
    ...
    def get_dps(self):
        ...
        effective_fire_rate = self.fire_rate / self.fire_rate_modifier
        return total_damage / effective_fire_rate
...
class MafiaTower(Tower):
    ...
    def draw(self, surf, selected=False):
        ...
        if progress > 0.3:
            money_text = FONT_TOWER.render(f"+{format_number(self.cash_per_interval)}", True, (255, 215, 0))
...
class FrostBlastTower(Tower):
    def __init__(self, x, y):
        super().__init__(x, y)
        self.name = "Frost Blast Square"
        self.type_name = "Ranger"
        self.cost = 1800
        self.total_invest = self.cost
        self.color = (160, 220, 255)
    ...
    def upgrade(self):
        ...
        if self.level == 2:
            self.damage += 3
            self.range += 15
            self.freeze_threshold -= 1
            self.max_hp += 100
        elif self.level == 3:
            self.damage += 4
            self.range += 20
            self.slow_per_hit += 0.01
            self.max_hp += 150
        elif self.level == 4:
            self.damage += 7
...
def draw_upgrade_ui(screen, tower):
    ...
    base_dps = (tower.damage * tower.bullet_count) / tower.fire_rate
    current_dps = tower.get_dps()
    
    stats_text = [
        f"Level    : {tower.level}",
        f"Type     : {tower.type_name}",
        f"DPS      : {current_dps:.1f}",
    ...
    if tower.level < tower.max_level:
        ...
        cost_surface = font_small.render(f"Cost: {format_number(upgrade_price)}", True, BLUE_COST)
    ...
    sell_text = font_small.render(f"Sell ({format_number(sell_price)})", True, WHITE)
...
# ------------------------------------------------
# SLINGSHOT TOWER - Splash Damage (Rare dari Spin)
# ------------------------------------------------
class SlingdroneTower(Tower):
    def __init__(self, x, y):
        super().__init__(x, y)
        self.name = "Slingdrone Square"
        self.cost = 420
        self.total_invest = self.cost
        self.size = 34
        ...
        self.range = 280
        self.damage = 320
        self.splash_damage = 160
        self.splash_radius = 60
        self.fire_rate = 1.8
        self.max_hp = 180
        self.hp = self.max_hp
        self.type_name = "Splash"
        self.level = 1
        self.max_level = 4
        self.cooldown = 0
        self.projectiles = []
        self.explosions = []
        self.base_explosion_surf = pygame.Surface(
            (self.splash_radius * 2, self.splash_radius * 2),
            pygame.SRCALPHA
        )
        pygame.draw.circle(
            self.base_explosion_surf,
            (255, 120, 0, 180),
    ...
    def update(self, dt, enemies):
        ...
        for proj in self.projectiles[:]:
            proj["x"] += proj["dx"] * dt * 15
            proj["y"] += proj["dy"] * dt * 15
            ...
            if dist_to_target < 10:
                for enemy in enemies:
                    dist = math.hypot(
                        enemy.x - proj["target_x"],
                        enemy.y - proj["target_y"]
                    )
            ...
            elif abs(proj["x"] - self.x) > 800 or abs(proj["y"] - self.y) > 800:
                self.projectiles.remove(proj)
    ...
    def draw(self, surf, selected=False):
        for proj in self.projectiles:
            for i in range(3):
                alpha = int(140 * (1 - i / 3))
                trail = pygame.Surface((12, 12), pygame.SRCALPHA)
                pygame.draw.circle(trail, (160, 100, 50, alpha), (6, 6), 5 - i)
                surf.blit(
                    trail,
                    (
                        int(proj["x"] - proj["dx"] * i * 2 - 6),
                        int(proj["y"] - proj["dy"] * i * 2 - 6)
                    )
                )

            pygame.draw.circle(surf, (100, 60, 30),
                               (int(proj["x"]), int(proj["y"])), 7)
            pygame.draw.circle(surf, (160, 100, 50),
                               (int(proj["x"]), int(proj["y"])), 5)

        for explosion in self.explosions:
            progress = explosion["timer"] / explosion["max_time"]
            scale = 1.0 + (1.3 - 1.0) * (1 - progress)
            alpha = int(200 * progress)
            scaled_radius = int(self.splash_radius * scale)
            explosion_surf = pygame.transform.smoothscale(
                self.base_explosion_surf,
                (scaled_radius * 2, scaled_radius * 2)
            )

            explosion_surf.set_alpha(alpha)

            surf.blit(
                explosion_surf,
                (
                    int(explosion["x"] - scaled_radius),
                    int(explosion["y"] - scaled_radius)
                )
            )
...
# ------------------------------------------------
# VOID REAPER TOWER - Ultimate Mythic Tower
# ------------------------------------------------
class VoidPulseTower(Tower):
    def __init__(self, x, y):
        super().__init__(x, y)
        self.name = "Void Pulse"
        self.cost = 3000
        self.total_invest = self.cost
        self.size = 45
        ...
        self.max_hp = 500
        self.hp = self.max_hp
        self.type_name = "Mythic"
        self.bullet_count = 1
        self.slow_amount = 0.4
        self.slow_duration = 180
        self.area_damage_radius = 120
        self.level = 1
        self.max_level = 4
        self.upgrade_info = {
            2: ["+900 Damage", "+80 Range", "Fire Rate +12%", "+20 Area", "+150 HP"],
            3: ["+1400 Damage", "+120 Range", "Fire Rate +18%", "+40 Area", "+250 HP"],
            4: ["+2200 Damage", "+180 Range", "Fire Rate +28%", "+60 Area", "+400 HP"],
        }
        self.upgrade_cost = {
            1: 4500,
            2: 7000,
    ...
    def update(self, dt, enemies):
        self.aura_pulse = (self.aura_pulse + dt * 3) % (2 * math.pi)
        ...
        for blast in self.void_blasts[:]:
            blast["lifetime"] -= dt
            if blast["lifetime"] <= 0:
                self.void_blasts.remove(blast)
        ...
        for enemy in enemies:
            ...
            if dist <= self.area_damage_radius:
                enemy.hp -= self.damage
                if not hasattr(enemy, 'slow_effect_timer'):
                    enemy.slow_effect_timer = 0
                enemy.slow_effect_timer = max(enemy.slow_effect_timer, self.slow_duration)
                if not hasattr(enemy, 'slow_factor'):
                    enemy.slow_factor = 1.0
                enemy.slow_factor = 1.0 - self.slow_amount
        
        self.void_blasts.append({
            "x": target.x,
            "y": target.y,
            "lifetime": 0.5,
            "max_lifetime": 0.5
    ...
    def upgrade(self):
        ...
        if self.level == 2:
            ...
            self.range += 180
            self.fire_rate *= 0.72
            self.area_damage_radius += 60
            self.max_hp += 400
            self.hp += 400
    
    def draw(self, surf, selected=False):
        for blast in self.void_blasts:
            progress = 1 - (blast["lifetime"] / blast["max_lifetime"])
            radius = int(self.area_damage_radius * progress)
            alpha = int(150 * (1 - progress))

            blast_surf = pygame.Surface((radius*2, radius*2), pygame.SRCALPHA)
            pygame.draw.circle(blast_surf, (150, 0, 200, alpha), (radius, radius), radius)
            pygame.draw.circle(blast_surf, (200, 0, 255, alpha//2), (radius, radius), radius//2)
            surf.blit(blast_surf, (int(blast["x"] - radius), int(blast["y"] - radius)))

        if selected:
            pulse_radius = int(self.range + math.sin(self.aura_pulse) * 10)
            pygame.draw.circle(surf, (150, 0, 200), (int(self.x), int(self.y)), pulse_radius, 3)
        else:
            aura_size = int(self.size + math.sin(self.aura_pulse) * 5)
            aura_surf = pygame.Surface((aura_size*2, aura_size*2), pygame.SRCALPHA)
            pygame.draw.circle(aura_surf, (150, 0, 200, 50), (aura_size, aura_size), aura_size)
            surf.blit(aura_surf, (int(self.x - aura_size), int(self.y - aura_size)))

        pygame.draw.rect(surf, self.color, 
                        (self.x - self.size//2, self.y - self.size//2, self.size, self.size),
                        border_radius=6)

        inner_size = self.size - 10
        pygame.draw.rect(surf, (200, 0, 255), 
                        (self.x - inner_size//2, self.y - inner_size//2, inner_size, inner_size),
                        border_radius=4)
        ...
        if self.hp < self.max_hp:
            ...
            hp_ratio = self.hp / self.max_hp
            pygame.draw.rect(surf, (200, 0, 255), (bar_x, bar_y, bar_w * hp_ratio, bar_h))
...
TOWER_TYPES = {
    ...
    "Slingdrone (420)": SlingdroneTower,
    "Void Pulse (3K)": VoidPulseTower,
}
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if start_btn_rect.collidepoint(pos) and len(state_data['selected_towers']) == 4:
                # Apply temporary money boost from spin rewards
                bonus = apply_temp_money_boost(state_data)
                if bonus > 0:
                    print(f"Money boost applied: +{bonus} for this game!")
...
def draw_lose_popup(screen, state_data):
    ...
    shape_obj = font_small.render(f"+{format_number(total_reward)} Shape", True, (100, 255, 255))
...
def chapter1_screen(state_data):
    ...
    data = game_data[1]
    money_text = font_small.render(f"Money: {format_number(state_data['money'])}", True, YELLOW)
    hp_text = font_small.render(f"Base HP: {format_number(state_data['base_hp'])}", True, (0, 255, 0))
    ...
    for t in list(state_data['towers']):
        # Apply boost to tower
        apply_boost_to_tower(t, active_boost)
...
def game_screen():
    ...
    if data["game_type"] == "td":
        text2 = font_small.render(f"Money: {format_number(data['money'])}", True, YELLOW)
        screen.blit(text2, (50, 180))

        text3 = font_small.render(f"Base HP: {format_number(data['base_hp'])}", True, (0, 255, 0))
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if start_btn_rect.collidepoint(pos) and len(event_data['selected_towers']) == 4:
                # Apply temporary money boost from spin rewards
                bonus = apply_temp_money_boost(event_data)
                if bonus > 0:
                    print(f"Money boost applied to Winter Event: +{bonus} for this game!")
                event_state = "playing"
                start_winter_map()
...
def draw_win_popup_winter(screen, state_data):
    ...
    if not state_data.get("win_reward_given", False):
        ...
        if not towers_data["Frost Blast (1,8K)"]["owned"]:
            towers_data["Frost Blast (1,8K)"]["owned"] = True
            save_inventory()

            state_data["got_frost_blast"] = True
    ...
    if state_data.get("got_frost_blast", False):
        tower_text = font_small.render(
            "Tower Baru Dibuka: Frost Blast!",
            True,
            (255, 200, 120)
        )
        screen.blit(
...
def winter_event_gameplay(event_data):
    ...
    money_text = font_small.render(f"Money: {format_number(event_data['money'])}", True, YELLOW)
    hp_text = font_small.render(f"Base HP: {format_number(event_data['base_hp'])}", True, (0, 255, 0))
    screen.blit(money_text, (40, 80))
    screen.blit(hp_text, (40, 120))
    ...
    for e in list(event_data['enemies']):
        if isinstance(e, GrandShaman) and e.to_delete:
            event_data['enemies'].remove(e)
            continue

        if isinstance(e, GrandShaman) and e.death_anim_active:
...
def reset_event_data():
    # Reset boss HP bar saat game direset
    boss_hp_bar.reset()
